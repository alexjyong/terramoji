...
- [ ] **T34** Add tests for unit spawning, movement, terrain restrictions, and settling
  ...
  - [x] **T34h** Test `moveUnits()` — verify toroidal wrap-around at map edges
  - [x] **T34i** Test `settleUnit()` — verify unit settles on empty cell, creates civ at unit's stage
  - [x] **T34j** Test `settleUnit()` — verify unit disappears when settling on existing civ cell
  - [x] **T34k** Test unit restTicks — unit rests 1 tick on spawn before moving
...
- [x] **T37** Run full test suite, fix any failures
  - _Files: `tests/test_simulation.js`, `tests/test_renderer.js`_

  - [x] **T37a** Run `tests/test_simulation.js` — verify all existing tests pass (T31–T33 + new T34/T35)
  - [x] **T37b** Run `tests/test_renderer.js` — verify all existing tests pass (T3 + new T36)
  - [x] **T37c** Fix any failing tests (if any)
